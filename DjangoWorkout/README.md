...
4. Run `make run` to start PostgreSQL in Docker, apply Django migrations, ensure a Django superuser exists, then launch the backend and frontend locally.

The backend runs on `http://localhost:8000`, the Django admin runs on `http://localhost:8000/admin/`, the frontend runs on `http://localhost:5173`, and PostgreSQL runs in Docker on `localhost:5432` using the credentials from `.env`. `make run` also prints the configured Django superuser credentials from `.env`.
...
- `make ensure-superuser` creates or updates the Django admin user from `.env`.
...
- `make run` starts PostgreSQL, runs migrations, ensures the Django admin user exists, then starts backend and frontend together.
